...
Three fields are required for the application to correctly index a dataset:
  ...
  4. **Maintenance update frequence**: The `maintenance` element should contain the planned maintenance update frequency for the dataset as specified by [EML standards](https://eml.ecoinformatics.org/schema/eml-dataset_xsd#DatasetType_maintenance). In particular, the `maintenanceUpdateFrequency` leaf element should be present and should be a valid [maintenance update frequency type controlled value](https://eml.ecoinformatics.org/schema/eml-dataset_xsd.html#MaintUpFreqType). If this value is absent, then a default value of `unknown` is set.
