...
7. `REGEX()` is supported via its 2-argument form (Ontop translates it to DuckDB's native regex matching). Use it for pattern matching that `CONTAINS`/`STRSTARTS` can't express.
